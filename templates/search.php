...
if($q) { 
	...
	$selector = "title|tag|summary|body~=$q, limit=50, template=article|tag|lesson"; 
...
?>
		...
		<header flex="100">
			...
			<span>
				Found <?=$matches->count?> Articles, Lessons or Tags
			</span>
		</header>
		<?php foreach ($matches as $match): ?>
			<article flex="60" flex-sm="100">
				<h3><a href="<?=$match->httpUrl?>"><?=$match->title?></a></h3>
				<?=$match->summary?>
				<?=$match->template=='tag' ? 'A list of all posts tagged with '.$match->title : '' ?>
				<?=tags($match)?>
			</article>
		<?php endforeach; ?>
<?php
